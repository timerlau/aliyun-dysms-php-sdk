...
class Sms
{
    ...
    public function multiSend()
    {
        ...
        $request->setPhoneNumberJson(json_encode($this->phone, JSON_UNESCAPED_UNICODE));

        // 必填:短信签名-支持不同的号码发送不同的短信签名
        $request->setSignNameJson(json_encode($this->sign, JSON_UNESCAPED_UNICODE));
        ...
        $request->setTemplateParamJson(json_encode($this->assign, JSON_UNESCAPED_UNICODE));

        // 可选-上行短信扩展码(扩展码字段控制在7位或以下，无特殊需求用户请忽略此字段)
        if ($this->extendCode && !empty($this->extendCode)) {
            $request->setSmsUpExtendCodeJson(json_encode($this->extendCode, JSON_UNESCAPED_UNICODE));
        }
